...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    if (!profile)
      return res.status(400).json({ msg: 'No matching profile found!' });
    res.json(profile);
  } catch (err) {
    console.error(err.message);
...
// @desc Delete user profile, users, posts
// @access Private
...
router.put(
  ...
  async (req, res) => {
    ...
    // update object to submit data that the user submits
    const newExp = {
      title,
      company,
      location,
...
router.put(
  ...
  [
    ...
    [
      ...
      check('fieldofstudy', 'Field of study is required is required')
        .not()
        .isEmpty(),
      check('from', 'From date is required').not().isEmpty(),
    ],
